...
export const AESTHETIC_EVALUATION = {
    ...
    dimensions: {
        harmony: {
            ...
            calculate: (factors) => {
                return ((factors.colorHarmony || 0) * 0.4 +
        ...
        balance: {
            ...
            calculate: (factors) => {
                return ((factors.visualWeight || 0) * 0.4 +
        ...
        contrast: {
            ...
            calculate: (factors) => {
                return ((factors.hierarchicalContrast || 0) * 0.4 +
        ...
        rhythm: {
            ...
            calculate: (factors) => {
                return ((factors.spacingRhythm || 0) * 0.4 +
        ...
        proportion: {
            ...
            calculate: (factors) => {
                return ((factors.sizeRelationships || 0) * 0.4 +
        ...
        unity: {
            ...
            calculate: (factors) => {
                return ((factors.designLanguage || 0) * 0.4 +
    ...
    evaluate: {
         ...
         */
        assess: (dimensionScores, targetLevel = "minimum") => {
            ...
            const passed = overall >= threshold.overall &&
                Object.entries(dimensionScores).every(([dimension, score]) => score >= threshold.dimensions[dimension]);
            // Identify strengths and weaknesses
            const strengths = [];
            const weaknesses = [];
            const recommendations = [];
            Object.entries(dimensionScores).forEach(([dimension, score]) => {
                const thresholdScore = threshold.dimensions[dimension];
                if (score >= thresholdScore) {
                    strengths.push(`${dimension}: ${score.toFixed(2)} (good)`);
                }
                else {
                    weaknesses.push(`${dimension}: ${score.toFixed(2)} (below ${thresholdScore})`);
                    recommendations.push(...AESTHETIC_EVALUATION.improvements[dimension]);
                }
            });
 ...
 */
export function evaluateAesthetic(dimensionScores, targetLevel = "minimum") {
    return AESTHETIC_EVALUATION.evaluate.assess(dimensionScores, targetLevel);
}
//# sourceMappingURL=layer-18-aesthetic-evaluation.js.map