 ...
 */
export function getRefactoringForSmell(smellName) {
    const smell = REFACTORING_QUALITY.codeSmells[smellName];
    if (!smell)
        return [];
    // Map code smells to refactoring techniques
    const techniques = [];
    smell.refactoring.forEach(refactoringName => {
        const technique = REFACTORING_QUALITY.refactoringTechniques[refactoringName.toLowerCase().replace(/\s+/g, '')];
        if (technique) {
...
//# sourceMappingURL=layer-25-refactoring-quality.js.map