 ...
 */
import { validateDimensions, validateImageBuffer } from './utils/validation';
 ...
 */
export async function executePipeline(params, config) {
    const errors = [];
    let asset = null;
    let metadata = null;
    let validation = null;
            ...
            if (useLocalModels) {
                try {
                    ...
                    if (process.env.STABILITY_AI_API_KEY || process.env.STABLE_DIFFUSION_API_KEY) {
                        ...
                        throw new Error(`Local Stable Diffusion not available and no API key provided. ` +
 ...
 */
export function validateAsset(asset, standards) {
    const errors = [];
    const warnings = [];
    let score = 1.0;
    // Validate format
    ...
    // Validate dimensions
    const dimValidation = validateDimensions(asset.metadata.dimensions.width, asset.metadata.dimensions.height, standards.minDimensions.width, standards.minDimensions.height, standards.maxDimensions.width, standards.maxDimensions.height);
 ...
 */
export function extractMetadata(asset) {
 ...
 */
export function generateCodeBindings(metadata, framework) {
    const bindings = [];
 ...
 */
export async function integrateAsset(asset, codeBindings, projectPath) {
    ...
    try {
        ...
        const assetEntry = {
            id: asset.metadata.id,
            path: assetPath,
            type: asset.type === 'sprite' ? 'sprite' : asset.type,
            spec,
            validated: true,
...
//# sourceMappingURL=layer-39-asset-pipeline.js.map