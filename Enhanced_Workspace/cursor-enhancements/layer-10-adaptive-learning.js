...
export const ADAPTIVE_LEARNING = {
    ...
    successMetrics: {
        ...
        calculateOverall: (metrics) => {
            ...
            let totalWeight = 0;
            // Quality (30% weight)
            const qualityScore = ((metrics.quality.passedPolishChecklist ? 1 : 0) * 0.3 +
                (metrics.quality.constraintRespected ? 1 : 0) * 0.4 +
                (metrics.quality.designAligned ? 1 : 0) * 0.3);
            score += qualityScore * 0.3;
            totalWeight += 0.3;
            // Alignment (40% weight)
            const alignmentScore = ((metrics.alignment.userApprovedWithoutModification ? 1 : 0) * 0.5 +
            ...
            const efficiencyScore = ((1 - Math.min(metrics.efficiency.iterationsNeeded / 5, 1)) * 0.4 +
    ...
    patternMatching: {
        ...
        match: (request, context, patterns) => {
 ...
 */
export function updatePatternSuccessRate(pattern, feedback) {
 ...
 */
export function findMatchingPatterns(request, context, patterns) {
...
//# sourceMappingURL=layer-10-adaptive-learning.js.map