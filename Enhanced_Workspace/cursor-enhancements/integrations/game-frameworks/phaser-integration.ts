    ...
    const animations = config.animations.map(anim => {
      ...
      if (Array.isArray(anim.frames)) {
        if (anim.frames.length === 1) {
          ...
          const isConsecutive = anim.frames.every((val, idx, arr) => 
...
${animations}${defaultAnimCode}`;
   ...
   */
  generateSpriteLoader(spriteSheets: Array<{ name: string; path: string; frameWidth: number; frameHeight: number; spacing?: number; margin?: number }>): GeneratedCode {
  ...
  generateGameConfig(config: {
    ...
    physics?: { type: "arcade" | "matter"; gravity?: { x: number; y: number }; debug?: boolean };
    scenes: string[];
    scale?: {
...
export default game;`;
   ...
   */
  private generateLoadAssets(config: SceneConfig): string {
    const assets: string[] = [];
   ...
   */
  private generateCreateContent(config: SceneConfig): string {
    const lines: string[] = [];
   ...
   */
  private generateUpdateLogic(config: SceneConfig): string {
    const lines: string[] = [];
 ...
 */
export function createPhaserIntegration(): PhaserIntegration {
