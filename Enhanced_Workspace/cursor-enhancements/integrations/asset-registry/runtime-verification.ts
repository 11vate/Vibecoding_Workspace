...
export interface RuntimeVerificationCode {
  ...
  checks: string[]; // Asset IDs that are checked
}

/**
 * Generate runtime checks for assets
 */
export function generateRuntimeChecks(
  assets: AssetEntry[],
  framework: GameFramework
): RuntimeVerificationCode {
  const checks: string[] = assets.map(a => a.id);
 ...
 */
function generateGenericChecks(
  assets: AssetEntry[],
  checks: string[]
): RuntimeVerificationCode {
  const codeLines: string[] = [
 ...
 */
export function createAssetLoaderWithVerification(
