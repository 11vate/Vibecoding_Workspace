 ...
 */
export const ASSET_SPEC_TEMPLATES: Record<string, AssetSpecTemplate> = {
  sprite_idle: {
    id: 'sprite_idle',
    name: 'Idle Sprite',
    type: 'sprite',
 ...
 */
export function extractAssetSpecs(code: string): AssetSpec[] {
  ...
  while ((match = jsonSpecPattern.exec(code)) !== null) {
    try {
      const spec = JSON.parse(match[1]) as AssetSpec;
 ...
 */
function parseCommentSpec(comment: string): AssetSpec | null {
  ...
  const type = parts[1] as AssetSpec['type'];
 ...
 */
function validateSpecStructure(spec: unknown): spec is AssetSpec {
 ...
 */
function inferSpecFromLoad(assetId: string, loadType: string, code: string): AssetSpec | null {
  const spec: AssetSpec = {
    assetId,
 ...
 */
export function validateAssetSpec(spec: AssetSpec): SpecValidationResult {
  const errors: string[] = [];
 ...
 */
export function applyTemplate(spec: AssetSpec, templateId: string): AssetSpec {
  const template = ASSET_SPEC_TEMPLATES[templateId];
  if (!template) {
    return spec;
  }

  const templated: AssetSpec = {
    ...template.defaults,
    ...spec
  };
 ...
 */
export function generateSpecFromDescription(
  description: string,
 ...
 */
function generateAssetId(description: string): string {
 ...
 */
function inferAssetType(description: string): AssetSpec['type'] {
 ...
 */
export function completeSpec(spec: Partial<AssetSpec>): AssetSpec {
  const completed: AssetSpec = {
