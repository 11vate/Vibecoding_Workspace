...
import type { PipelineConfig, PipelineResult } from '../../layer-39-asset-pipeline';
 ...
 */
export async function autoGenerateAsset(
  spec: AssetSpec,
  ...
  try {
    // Execute pipeline
    const result: PipelineResult = await executePipeline(pipelineParams, pipelineConfig);
    ...
    // Save asset to disk
    const assetPath = await saveAssetToDisk(
      result.asset,
      completedSpec,
      config.projectPath
    );
...
async function generateWithFallback(
  ...
  try {
    ...
    errors.push(
      `Procedural generation failed: ${proceduralError instanceof Error ? proceduralError.message : 'Unknown error'}`
    );
  }
 ...
 */
function specToPipelineParams(spec: AssetSpec): SpriteGenerationParams {
  // Parse resolution
 ...
 */
async function generateProcedurally(
  spec: AssetSpec,
  config: AssetEnforcementConfig
 ...
 * Save asset to disk
 */
async function saveAssetToDisk(
  asset: GeneratedAsset,
  spec: AssetSpec,
  projectPath: string
): Promise<string> {
 ...
 */
function getDefaultAssetPath(spec: AssetSpec): string {
  const extension = 'png';
  let dir = 'assets';
 ...
 */
function createErrorAsset(spec: AssetSpec): GeneratedAsset {
  return {
    type: spec.type === 'sprite_sheet' ? 'sprite' : spec.type as any,
    data: Buffer.alloc(0),
    metadata: {
