...
export interface VerificationReport {
  ...
  assetsFound: number; // Number of asset references found
  assetsValid: number;
 ...
 */
export async function verifyProjectAssets(projectPath: string): Promise<VerificationReport> {
  ...
  try {
    ...
    const allReferences = new Map<string, Set<string>>(); // assetId -> file paths
 ...
 */
export async function repairMissingAssets(
      ...
      if (generationResult.success) {
        ...
        const assetEntry = {
          ...
          references: Array.from(
            new Set(report.brokenReferences.filter(ref => ref.assetId === assetId).map(ref => ref.filePath))
          )
        };
 ...
 */
export async function generateVerificationReport(projectPath: string): Promise<string> {
