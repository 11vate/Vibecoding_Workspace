...
export interface VisionAnalysisRequest {
  image: Buffer | string; // Base64 or URL
  prompt: string;
...
export class OpenAIVisionService {
  private config: Required<Omit<OpenAIVisionConfig, 'retryOptions' | 'rateLimiter'>> & {
    retryOptions?: RetryOptions;
    rateLimiter?: RateLimiter;
  };
   ...
   */
  private prepareImageData(image: Buffer | string): string {
   ...
   */
  async analyzeImage(request: VisionAnalysisRequest): Promise<ImageAnalysis> {
    ...
    if (!this.config.apiKey) {
      throw new OpenAIVisionError(
        "OpenAI API key is required. Set OPENAI_API_KEY or enable local models with USE_LOCAL_MODELS=true"
      );
    }
        ...
        ? `${request.prompt}\n\nReturn your analysis as a JSON object with the following structure:
{
  ...
  "format": "png|jpeg|webp|unknown"
      ...
      const data = await this.makeRequest(payload) as {
...
Return structured analysis as JSON.`;
    ...
    try {
      ...
      throw new OpenAIVisionError(
        `Failed to analyze sprite sheet: ${error instanceof Error ? error.message : 'Unknown error'}`,
        undefined,
        undefined,
        error
      );
   ...
   */
  private parseAnalysis(content: string, isJson: boolean = false): ImageAnalysis {
    let parsed: unknown;
   ...
   */
  private convertToSpriteSheetAnalysis(analysis: ImageAnalysis, imageBuffer: Buffer): SpriteSheetAnalysis {
 ...
 */
export function createOpenAIVisionService(config: OpenAIVisionConfig): OpenAIVisionService {
