...
export class StableDiffusionService {
    config;
    defaultBaseUrl = "https://api.stability.ai/v1/generation/stable-diffusion-xl-1024-v1-0/text-to-image";
    defaultRateLimiter;
    constructor(config) {
    ...
    validateParams(params) {
        // Validate dimensions
        const dimValidation = validateDimensions(params.resolution[0], params.resolution[1], 16, // Min width
        16, // Min height
        2048, // Max width
        2048 // Max height
        );
    ...
    async makeRequest(payload) {
        ...
        const result = await retry(async () => {
            ...
            if (!response.ok) {
                ...
                const error = new StableDiffusionError(errorData.message || `API error: ${response.statusText}`, response.status, errorData.name, errorData);
                throw error;
            }
    ...
    async generateImage(params) {
        ...
        if (!this.config.apiKey && process.env.USE_LOCAL_MODELS !== "false") {
            try {
                ...
                const localSD = createLocalStableDiffusionService({
                    type: (process.env.SD_TYPE || "comfyui"),
        ...
        if (!this.config.apiKey) {
            throw new StableDiffusionError("Stability AI API key is required. Set STABILITY_AI_API_KEY or enable local models with USE_LOCAL_MODELS=true");
        }
        try {
            ...
            const data = await this.makeRequest(sdParams);
            // Validate response
            const validation = GenerationResponseSchema.safeParse(data);
            if (!validation.success) {
                throw new StableDiffusionError('Invalid response format from API', undefined, undefined, validation.error);
            }
            ...
            throw new StableDiffusionError(`Failed to generate image: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, undefined, error);
        }
    }
    /**
    ...
    async generateSprite(params) {
        try {
            ...
            throw new StableDiffusionError(`Failed to generate sprite: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, undefined, error);
        }
    }
    /**
 ...
 */
export function createStableDiffusionService(config) {
