...
export class GraphManager {
  private config: GraphManagerConfig;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await this.config.vectorDB.createCollection(
        this.config.collectionName!,
        1536 // Default embedding dimension
      );
    } catch (error) {
      throw new GraphManagerError(
        `Failed to initialize graph: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error
      );
    }
  }
   ...
   */
  async createNode(node: Omit<KnowledgeNode, 'embedding'>): Promise<KnowledgeNode> {
    try {
   ...
   * Create relationship
   */
  async createRelationship(edge: KnowledgeGraphEdge): Promise<void> {
  ...
  async search(query: SemanticQuery): Promise<SearchResult[]> {
    try {
      ...
      throw new GraphManagerError(
        `Failed to search graph: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error
      );
    }
  }
   ...
   */
  async validateCanon(nodeId: string, constraints: string[]): Promise<{ valid: boolean; violations: string[] }> {
   ...
   */
  async extractPatterns(nodeType: string, minSimilarity: number = 0.7): Promise<KnowledgeNode[][]> {
    try {
      ...
      throw new GraphManagerError(
        `Failed to extract patterns: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error
      );
    }
  }
   ...
   */
  private extractTextContent(node: KnowledgeNode | Omit<KnowledgeNode, 'embedding'>): string {
