...
import type { AssetRegistry, AssetEntry } from './integrations/asset-registry/asset-registry';
import type { AssetSpec } from './integrations/asset-registry/asset-registry';
import type { MissingAsset, ScanResult } from './utils/asset-reference-scanner';
...
export interface AssetEnforcementConfig {
  ...
  autoGenerate: boolean; // Auto-generate missing assets
  strictMode: boolean; // Block code generation if assets missing
  validateOnDisk: boolean; // Verify files exist on disk
}
...
export async function validateAssetReferences(
  ...
  if (missingAssets.length > 0) {
    if (config.strictMode) {
      canProceed = false;
      errors.push(
        `Cannot proceed: ${missingAssets.length} asset(s) are missing. ` +
        `Assets must exist before code can reference them.`
      );
    } else {
      warnings.push(
        `${missingAssets.length} asset(s) are missing. ` +
        (config.autoGenerate 
 ...
 */
export async function enforceAssetExistence(
  assetIds: string[],
  config: AssetEnforcementConfig
      ...
      if (config.autoGenerate) {
        try {
          ...
          const assetEntry: AssetEntry = {
            id: assetId,
            path: generated.path,
            type: spec.type === 'sprite_sheet' ? 'sprite' : spec.type as any,
            spec,
            validated: true,
          ...
          errors.push(
            `Failed to auto-generate asset ${assetId}: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      } else {
        errors.push(
          `Asset ${assetId} is missing and auto-generation is disabled. ` +
      ...
      try {
        ...
        if (config.autoGenerate) {
          try {
            ...
            errors.push(
              `Failed to regenerate asset ${assetId}: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
          }
        } else {
 ...
 */
export async function autoGenerateMissingAssets(
  specs: AssetSpec[],
  config: AssetEnforcementConfig
 ...
 */
export async function preCodeGenerationCheck(
  codeToGenerate: string,
  config: AssetEnforcementConfig
): Promise<{
  canProceed: boolean;
