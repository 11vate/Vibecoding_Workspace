 ...
 */
export function validateAssetMetadata(metadata: Partial<AssetMetadata>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (!metadata.path) errors.push("Missing required field: path");
  if (!metadata.format) errors.push("Missing required field: format");
 ...
 */
export function getOrganizationPattern(patternName: string): AssetOrganizationPattern | undefined {
  const patterns = ASSET_MANAGEMENT.organization.directoryStructure;
  return (patterns as Record<string, AssetOrganizationPattern>)[patternName];
}
