...
export const CODE_GENERATION_PATTERNS = {
  ...
  designPatterns: {
    creational: {
      singleton: {
        name: "Singleton",
        category: "creational" as PatternCategory,
      ...
      repository: {
        name: "Repository",
        category: "modern" as PatternCategory,
    ...
    behavioral: {
      strategy: {
        name: "Strategy",
        category: "behavioral" as PatternCategory,
 ...
 */
export function getDesignPattern(category: PatternCategory, patternName: string): DesignPattern | undefined {
  const categoryPatterns = CODE_GENERATION_PATTERNS.designPatterns[category];
  if (!categoryPatterns) return undefined;
  return (categoryPatterns as Record<string, DesignPattern>)[patternName];
}

/**
 * Get algorithm pattern by name
 */
export function getAlgorithmPattern(category: keyof typeof CODE_GENERATION_PATTERNS.algorithms, algorithmName: string): AlgorithmPattern | undefined {
  const categoryAlgorithms = CODE_GENERATION_PATTERNS.algorithms[category];
  if (!categoryAlgorithms) return undefined;
  return (categoryAlgorithms as Record<string, AlgorithmPattern>)[algorithmName];
}

/**
 * Get data structure pattern by name
 */
export function getDataStructurePattern(structureName: string): DataStructurePattern | undefined {
  return (CODE_GENERATION_PATTERNS.dataStructures as Record<string, DataStructurePattern>)[structureName];
}
